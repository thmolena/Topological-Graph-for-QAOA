...
initialization and does not surpass it; see the main table in the
...
> pip-installed PyTorch can each bundle an OpenMP library. The setting affects loader behavior
...
| --- | --- | --- |
| `results/summary.json` | `scripts/run.py` | Per-policy approximation ratio with 95% CIs, mean queries-to-target, target hit rate, the query-budget frontier curve, the per-family breakdown, and run provenance. The authoritative artifact for every table and figure. |
| `results/main_results.md` | `scripts/make_tables.py` | Markdown main table. |
| `results/main_results.tex` | `scripts/make_tables.py` | LaTeX main table. |
